...
-- RLS Policies for invoices (idempotent)
...
-- RLS Policies for invoice_items (idempotent)
DROP POLICY IF EXISTS "invoice_items_select" ON invoice_items;
CREATE POLICY "invoice_items_select" ON invoice_items FOR SELECT TO authenticated
  USING (invoice_id IN (SELECT id FROM invoices WHERE workspace_id IN (SELECT get_user_workspace_ids(auth.uid()))));
...
CREATE POLICY "invoice_items_insert" ON invoice_items FOR INSERT TO authenticated
  WITH CHECK (invoice_id IN (SELECT id FROM invoices WHERE workspace_id IN (SELECT get_user_workspace_ids(auth.uid()))));
